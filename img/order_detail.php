<style>
.image_loading{background-image:url("img/load.gif");background-size:auto auto;background-repeat:no-repeat;background-position:center;}
p.image_loading{margin-bottom:0;display:none;} </style>
<?php
...
if(isset($_POST["cupsubmit"])){
	...
	if ($con->query($sqlu) === TRUE) {/*echo "New record src created successfully";*/}
	else {/*echo "Error: " . $sqlu . "<br>" . $con->error;*/}
	}
			}
...
<div class="inner-section">
    ...
    <?php $sqlcomuim ="SELECT * FROM ph_users WHERE uid ='".$row_sb['uid']."'";
						$resultcomuim = $con->query($sqlcomuim);
        ...
        <p><?php echo $row_sb["request_date"];?></p>
			...
			$sqlckassgn ="SELECT * FROM ph_assign_project WHERE projects_id ='".$odid."'";
				$resultckassgn = $con->query($sqlckassgn);
					...
					$assignid = $rowckassgn['tmid'];
						...
						$resultassgnnm = $con->query($sqlassgnnm);
		   ...
		   $sqlusern ="SELECT * FROM ph_order_quote WHERE oid ='".$odid."'order by qid desc limit 1";
					$resultusern = $con->query($sqlusern);
                ...
                <table width="100%" cellspacing="0" cellpadding="0">
                    ...
                    <tbody><tr>
                    	<td>PSD to <?php echo $row_sb["select_options"]; ?></td>
           ...
           $sql_gcup = "SELECT * FROM ph_use_coupons WHERE order_id='$odid'";
			$result_gcup = mysqli_query($con, $sql_gcup);
			...
			$coupon_code = $row_gcup['coupon_code'];
					...
					$result_gcupm = mysqli_query($con, $sql_gcupm);
            ...
            <?php 	$sqlcomtal ="SELECT * FROM ph_order_chat WHERE oid ='".$odid."'order by chid ASC";
				$resultcomtal = $con->query($sqlcomtal);
...
<?php
    	...
    	<div class="container">
			...
			$result_fedbk = mysqli_query($con, $sql_fedbk);
			...
			$result_feeduser = mysqli_query($con, $sql_feeduser);
