...
$user_password= my_decryption($adpassword);
 ...
 if (isset($_POST['submit'])) {
   ...
   echo $newpass = $_POST["pass_word"];
	...
	$sql = "UPDATE ph_adminstrator SET adname='$name',ademail='$email',adpass='$passwordad' WHERE aid='$team'";
  if ($conn->query($sql) === TRUE) {/*echo "Record updated successfully";*/$locatoin = 'edit_team_member.php?userid='.$team.'&edit=true';}
  else {/*echo "Error updating record: " . $conn->error;*/$locatoin = 'edit_team_member.php?userid='.$team.'&edit=false';}
...
<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
      <h2 class="page-header">Edit Team Member</h2>
	  
<div class="col-sm-6 ">
<?php 
	 if($_REQUEST['edit']=='true'){
      echo '<div class="alert alert-success"><strong>Team Member Update! </strong> Successfully.</div>';
	}
elseif($_REQUEST['edit']=='false'){
      echo '<div class="alert alert-danger"><strong> Unable! </strong> to Update Team Member.</div>';
	}
?>
	 ...
	 <form id="editnuser" action="" method="post">
						...
						<input type="hidden" name="team" value="<?php echo $userid; ?>" />
